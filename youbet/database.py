...
class Round(Base):
    ...
    date_created = db.Column(db.DateTime, nullable=False, default=now)
    accept_wagers = db.Column(db.Boolean, nullable=False, default=False)
    odds = db.Column(db.String(30))
...
wager_stake = db.Table('wager_stake',
    db.Column('wager_id', db.Integer, db.ForeignKey('wager.id')),
    db.Column('user_id', db.Integer, db.ForeignKey('user.id'))
)
